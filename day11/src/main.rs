...
pub fn part1(input: &str) -> impl std::fmt::Display {
    let sec = input.split("\n\n");
    let mut monkeys = vec![];
    for (_i, se) in sec.enumerate() {
        let lines = se.lines().skip(1).collectvec();
        //   Starting items: 
        let items = lines[0][18..].split(", ").map(|x| x.parse::<i64>().unwrap()).collectvec();
        //   Operation: new = 
        let op = lines[1][19..].split_whitespace().skip(1).collectvec();
        //   Test: divisible by 
