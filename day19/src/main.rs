...
fn sat(costs: &[[i64; 4]; 4], r: &[i64; 4], i: usize) -> bool {
    r[0] >= costs[i][0] && r[1] >= costs[i][1] && r[2] >= costs[i][2] && r[3] >= costs[i][3]
}
...
pub fn part1(input: &str) -> impl std::fmt::Display {
    ...
    for line in input.lines() {
        ...
        let id = b.split_whitespace().collectvec()[1].parse::<i64>().unwrap();
        let sentences = s.split(". ");
        let robot_costs: [[i64; 4]; 4] = sentences
            .map(|s| {
                let words = s.split([' ', '.']).collectvec();
                let mut req = [0; 4];
